...
### 高可用
> 高可用本质就是搭建一个分布式系统，需要了解一些基础的[分布式理论](https://wqhuanm.github.io/Issue_Blog/2025/04/28/33_%E5%88%86%E5%B8%83%E5%BC%8F%E7%BB%8F%E5%85%B8%E7%90%86%E8%AE%BA%E5%8F%8A%E7%AE%97%E6%B3%95%E4%B8%80%E8%A7%88/)

- 主从复制 ：从节点用于备份主节点数据
    - redis的主从复制采用**读写分离** ：master主节点执行读写请求，而slave从节点只执行读请求；主节点会不断把写命令同步给从节点（追求ap可用，不保证主从数据的强一致性）
    - 数据同步策略
        - 每个节点都会记录一个`复制id`（runid，上次连接的主节点生成的）和`复制偏移量`（offset，接受到的主节点数据的偏移量）
            - 主节点会记录历史的runid
        - 如果从节点传递的runid在主节点中没有记录或者offset落后太多，则进行**全量复制**，同步RDB文件
        - 否则进行**增量复制**，同步近期的写命令
    - 哨兵(sentinel)机制 ：用于选举主节点（类似paxos算法）
        - 系统一般会部署几个哨兵节点，每个哨兵节点都会定期确认服务节点是否存活，如果不存活则标记为**主观下线**
        - 如果**主节点**被哨兵确认为主观下线，当前哨兵会向其他哨兵确认，如果大多数哨兵均判断主节点下线，则该哨兵认为主节点**客观下线**。同时该哨兵尝试成为哨兵leader来选举新的主节点
        - 判断主节点下线的哨兵会成为候选者，并向哨兵集群申请投票（每个哨兵只投一票），当哨兵获取大多数票数时成为主节点。leader哨兵选举主节点后会通知服务集群

- redis-cluster(数据分片) ：即水平扩展，使得整个redis集群可存储更多的数据
    - 数据分片实现
        - 划分出`2^14`个哈希槽，每个服务节点管理一定数量槽位
        - 客户端会维护每个槽位由哪个服务节点管理
        - 当集群发生动态伸缩时，槽位所属服务节点可能会变更
            - **Move永久重定向** ：当客户端请求key所属槽位不在当前节点时，返回move告知客户端对应槽位的新处理节点
            - **ASK临时重定向** ：当请求key所属槽位正在被迁移且当前key不在该节点时，返回ask告知相应的新节点（但是客户端不更新槽位对应的服务节点，因为迁移时，还有该槽位的部分key仍在当前节点）
    - 高可用实现
        - 集群节点含有多个主节点（每个主节点负责各自的分片），同时每个主节点有一定的从节点用于备份
        - 集群采用gossip思想交换节点间状态，当从节点检测到其主节点**客观下线**时，会向其他主节点申请投票，尝试成为新的新主节点
