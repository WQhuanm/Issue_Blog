...
1. 两阶段提案如何确定共识成立（两个多数派必相交）
    + 假设提议者a和提议者b的提案号和设定值分别为(1,va),(2,vb)，则接受者只会出现如下情况
        ...
        + 超过一半的接受者accept(1,va)：最终共识值一定是va。说明prepare(2)来晚了，虽然大多数接受者接受提案，但是这大多数接收者中必定有人返回了之前接受的va，所以提议者b最终提案的值从vb修改成了va
...
1. 如何成为leader
    ...
    1. 任何状态下，若如果收到>=自己当前任期的leader心跳包，说明是新leader，则Candidate/old leader把自己转为follower
