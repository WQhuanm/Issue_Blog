...
- 左值引用、右值引用、万能引用
    - 左值与右值
        - 左值 ：出现在赋值号左侧，**可以被取地址**的具名变量。如：`int a = 10;`， `a` 就是左值
            - 左值有持久的内存地址，因而支持`&`取地址操作
        - 右值 ：出现在赋值号右侧，**不可被取地址**的临时值（临时对象/字面量/函数返回值等）。如：`int a = 10;`， `10` 就是右值
            - 右值往往在表达式结束时消失，因此其临时内存地址无意义
        - 一个表达式的结果如果其内存地址固定，则为左值，否则为右值
            - 赋值、解引用操作(++a)等表达式结果均为左值（++a的结果就是自增后的a）
            - 引用、二元运算、(a++)等表达式均为右值（a++实际是临时副本拷贝a，然后自增a，表达式返回的是临时副本）
...
- const
    - 修饰变量
        - 顶层const ：**修饰 对象/指针 这些变量本身**，要求变量本身的值不能再改变
            - 如`const Stu a` 要求变量a的值（如成员变量）不能改变 
            - 如`Stu* const ptr`要求ptr的值（即指向的地址）不能再改变
        - 底层const ：**修饰指针/引用指向的数据**，要求指针/引用指向的内存数据不能通过当前指针/引用修改
...
- 函数和变量的 **声明（declaration）与 定义（definition）**
    - 声明 ：告诉编译器该函数/变量/类等的存在/类型，后续会进行定义。编译器会为声明留下链接符号，后续执行链接时，会把声明的链接指向定义所在的内存
        - 变量的声明使用extern修饰，如`extern int val;`（extern使得val只声明，但不分配内存）
    - 定义(声明+内存分配) ：告诉编译器**函数逻辑/变量类型**，编译器会为其分配**内存空间**（存放函数代码，或是分配变量内存）
        - 变量定义不一定要初始化赋值(`int val`时编译器就为其分配好内存了)
        - 变量可以赋值多次，但是定义只能有一次（而且定义必须声明变量类型）
    - 关于类的声明与定义
        - 类允许被重复定义，但是要多次定义都完全相同（方便类在头文件定义）
        - 但是类的成员方法/静态变量仍需要遵循一次定义且**必须进行定义**(否则链接时,外部符号找不到定义分配的内存)
            - 类成员方法被隐式视为inline函数, 所以可以在类内部定义(需要确保没有在其他地方进行定义,否则链接错误)
            - 静态变量则必须在类外部进行一次定义
...
- 分离式编译（Separate Compilation）
    ...
    - 具体实现 ：在头文件进行声明，在指定的唯一源文件进行定义
        ...
        class A{
            public:
                void A_print();
                static int val;//这里只是进行了声明，但是不会分配内存空间（即没有定义），所以不允许执行初始化
...
- 模板参数 ：`template<class T ,size_t N,int... args>void f(T a){}` （调用为 `f<int,1e5,1,2,3>(666);`）
    - 可以使用class/typename 声明该参数类型可任意，也可指定具体的参数类型，亦可使用...表示为可变参数
    - 泛型T的推导规则（auto推导规则也是类似的） ：因为参数默认是**值拷贝**
        - T推导时会忽略&（除非使用`T&`或`T&&`接收引用）
        - 顶层const、顶层volatile等修饰**变量本身**的操作也会被忽略，而不会被传递到拷贝变量上
        - 底层const会保留，保证指针/引用指向内存不被修改
