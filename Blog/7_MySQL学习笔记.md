...
1. 优化器：优化SQL语句，分析索引使用(如 调整where的语序，使得尽可能用上索引)
...
#### 4. innodb支持RC和RR隔离级别实现是用的一致性视图(consistent read view)
...
#### 5. 多个事务同时执行的时候，就可能出现脏读（dirty read）、不可重复读（non-repeatable read）、幻读（phantom read）的问题
...
+ 若B符合查询条件的索引，使用（ndex Nested-Loop Join，NLJ），复杂度n*log(m)
...
低效率SQL：SELECT * FROM 表 a,  where 条件 LIMIT 100000,20(跳过前面offset数据，只取后面20行);

+ 延迟关联（本质是索引覆盖）    
    SQL更改为：SELECT a.* FROM 表 1 a, (select id from 表 1 where 条件 LIMIT 100000,20 ) b where a.id=b.id
    1. 子查询在利用索引的同时，只查询符合条件的id，无需回表，效率高 
